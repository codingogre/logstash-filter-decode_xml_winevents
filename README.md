...
- Download plugin
```sh
wget https://github.com/codingogre/logstash-filter-decode_xml_winevents/raw/main/logstash-filter-decode_xml_winevents-1.0.2.gem
```
- Install plugin
```sh
# Logstash 2.3 and higher
cd to where logstash is installed
./bin/logstash-plugin install --no-verify /path/to/logstash-filter-decode_xml_winevents-1.0.2.gem
```
- Restart Logstash
```sh
